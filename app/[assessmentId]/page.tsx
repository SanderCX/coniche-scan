  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 px-6 py-16">
      <div className="text-center">
        ...
        <p className="mt-2 text-lg text-muted">{assessment.subtitel}</p>
        <p className="mx-auto mt-4 max-w-2xl text-muted">{assessment.beschrijving}</p>
        <p className="mt-3 text-sm text-muted">Bedoeld voor: {assessment.doelgroep}</p>

        <div className="mt-8 flex flex-wrap justify-center gap-3">
          <Link
            href={`/${assessment.id}/voorbeeld`}
            className="rounded-lg bg-brand px-6 py-3 text-sm font-semibold text-white transition hover:brightness-90"
          >
      ...
      <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-3">
        {assessment.featureCards.map((card) => (
          <div key={card.titel} className="rounded-2xl border border-gray-200 bg-white p-6">
            ...
            <p className="mt-2 text-sm text-muted">{card.tekst}</p>
          </div>
        ))}
      </div>
